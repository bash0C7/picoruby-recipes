...
DDJ-400とATOM Matrixでリアルタイムなドラムパフォーマンスとエフェクトコントロールを実現します。FILTERノブで残響（リバーブ）とコーラスを直感的に操作できます。
...
### ✨ DECK2 FILTER → コーラスコントロール
- FILTERノブを回すとコーラス（音の広がり・厚み）を10段階で調整
- 左回し：ドライ（広がりなし）
- 右回し：ウェット（広がり・厚みたっぷり）
...
### コーラスが効かない
- MIDI Unitが CC#93 (Chorus Send) に対応しているか確認
- コーラス機能を持つ音源が必要
...
[5] 🎵 CHORUS: Level 7 (raw:112 → 18)
```

## パフォーマンスのコツ

1. **残響は大胆に**: フィルインで一気に右端まで回す
2. **コーラスは表現的に**: タムの厚み調整で空間を演出
3. **両手使い**: 左手で残響、右手でコーラスをコントロール
